...
add_action('admin_menu', 'mbwpl_add_menu');
add_action('wp_footer', 'mbwpl_show_buttons');

function mbwpl_add_menu()
{

    add_menu_page('messenger-buttons-light', esc_attr__('Messenger Buttons light','MBWPL'), 'manage_options', WPMB_PATH.'admin/form.php');
}


function mbwp_is_mobile() {
...
function mbwpl_show_buttons()
{
    global $wpdb;


    $table = $wpdb->prefix . 'messenger_button';
    $sSQL = $wpdb->prepare("select * from $table WHERE active = 1", 1);
    ...
    <?php wp_enqueue_style('messenger_scc', plugin_dir_url(__FILE__) . 'assets/' . $data['position'] . '.css'); ?>
    <div class="">
        <div class="share-buttons-wrap" style="position: absolute">
            <div class="share-buttons-list">
                <?php foreach ($arrresult as $key => $val) { ?>
                    <div class="share-buttons-item">
                        ...
                        $href .= $val->code;
                        $href .=str_replace(' ', '', $number);
                        $href .= ($val->code == 'skype:') ? '?chat' : '';
                        ?>
                        <a target="_blank" href="<?php echo $href ?>" onclick="<?= $val->onclick; ?>">
                            <img style="width: 45px"
                                 src="<?= WPMB_IMAGE.mb_strtolower($val->title) ?>.png">
                        </a>
                    </div>
                <?php } ?>
            </div>
